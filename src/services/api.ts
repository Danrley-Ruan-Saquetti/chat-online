...
export class ApiCore {
    ...
    public async postForm<T = any, R = AxiosResponse<T>, D = any>(url: string, data?: D, config?: AxiosRequestConfig<D>) {
        const response = await this.api
            .postForm<T, R, D>(url, data, config)
            .then(res => res)
            .catch(err => err)
    ...
    public async patchForm<T = any, R = AxiosResponse<T>, D = any>(url: string, data?: D, config?: AxiosRequestConfig<D>) {
        const response = await this.api
            .patchForm<T, R, D>(url, data, config)
            .then(res => res)
            .catch(err => err)
    ...
    public async options<T = any, R = AxiosResponse<T, any>, D = any>(url: string, config?: AxiosRequestConfig<D>) {
        const response = await this.api.options<T, R>(url, config)

        return response
    }

    public async patch<T = any, R = AxiosResponse<T, any>, D = any>(url: string, data?: D, config?: AxiosRequestConfig<D>) {
        const response = await this.api.patch<T, R>(url, data, config)
